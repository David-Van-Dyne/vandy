...
class SourceComponent {
    ...
    get fullName() {
        ...
        if (this.parent && this.type.ignoreParentName) {
            if (!this.name) {
                throw new sfError_1.SfError(`Component was initialized without a name: ${this.xml} (${this.type.name})`);
            }
            return this.name;
        }
        else {
    ...
    parseFromParentXml(parentXml) {
        ...
        if (!matched) {
            throw new sfError_1.SfError(`Unable to find matching parent xml file for ${this.xml}`);
        }
        return matched;
    }
    ...
    getNonDecomposedChildren() {
        ...
        return Object.values(this.type.children.types).flatMap((childType) => {
            ...
            return elements.map((element) => {
                ...
                if (!name) {
                    throw new sfError_1.SfError(`Missing ${uniqueIdElement} on ${childType.name} in ${this.xml}`);
                }
