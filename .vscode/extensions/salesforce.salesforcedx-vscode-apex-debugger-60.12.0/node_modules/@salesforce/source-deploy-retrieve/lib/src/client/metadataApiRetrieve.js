    ...
    if (options.unzip) {
        ...
        for (const filePath of Object.keys(zip.files)) {
            ...
            if (!zipObj || zipObj?.dir) {
                graceful_fs_1.default.mkdirSync(path.join(extractPath, filePath), { recursive: true });
            }
            else {
                // eslint-disable-next-line no-await-in-loop
                const content = await zipObj?.async('nodebuffer');
                if (content) {
                    graceful_fs_1.default.writeFileSync(path.join(extractPath, filePath), content);
                }
            }
        }
    }
