...
// node_modules/@vscode/webview-ui-toolkit/node_modules/tslib/tslib.es6.mjs
...
// views/sidebar/sidebarView.ts
var vscode;
var SidebarView = {
  init: () => {
    vscode = acquireVsCodeApi();
    SidebarView.provideDesignSystem();
    window.addEventListener("load", SidebarView.listenerMain);
    window.addEventListener("message", SidebarView.listenerMessage);
  },
  //this is necessary to use the design components in SidebarViewProvider
  provideDesignSystem: () => {
  ...
  listenerMain: () => {
    ...
    askButton?.addEventListener("click", SidebarView.handleAskClick);
    const clearButton = document.getElementById("clear");
    clearButton?.addEventListener("click", () => {
      SidebarView.handleClearClick(true);
    });
    const walkthroughLink = document.querySelector(".walkthrough");
    walkthroughLink.addEventListener("click", () => {
      const message = { type: "WALKTHROUGH" /* walkthrough */ };
      vscode.postMessage(message);
    });
    const tcgLink = document.querySelector(".trigger-tcg");
    tcgLink.addEventListener("click", () => {
      const message = { type: "TCG" /* tcg */ };
      vscode.postMessage(message);
    });
    const questionTextArea = document.getElementById("question");
    questionTextArea.addEventListener("input", () => {
      const isEmpty = questionTextArea.value.trim() === "";
      SidebarView.enableButtons(!isEmpty);
    });
    questionTextArea.addEventListener("keyup", (event) => {
      SidebarView.handleEnterKeyPress(event);
    });
  },
  listenerMessage: (event) => {
    ...
    switch (message.type) {
      case "RESPONSE" /* response */:
        if (message.data) {
          SidebarView.setResponse(message.data);
        }
        copyLinks.forEach((link) => {
          link.addEventListener("click", SidebarView.handleCopyClick);
        });
        SidebarView.toggleProgress(false);
        SidebarView.enableButtons(true);
        break;
      case "REACTION" /* reaction */:
        SidebarView.handleIconSelection(message);
        break;
      case "ERROR" /* error */:
        SidebarView.handleClearClick(false);
        break;
      default:
        break;
    }
  ...
  handleCopyClick: (event) => {
    const copyLink = event.currentTarget;
  ...
  handleAskClick: () => {
    SidebarView.enableButtons(false);
    SidebarView.toggleResponse({ hideAll: true });
    SidebarView.toggleProgress(true);
  ...
  handleClearClick: (sendTelemetry = true) => {
    const question = document.getElementById("question");
    question.value = "";
    if (sendTelemetry) {
      const message = {
  ...
  handleEnterKeyPress: (event) => {
    ...
    if (event.altKey) {
      const questionTextArea = document.getElementById("question");
      questionTextArea.value = questionTextArea.value + "\r\n";
  ...
  toggleResponse: ({ hideCodeResponse = false, hideAll = false }) => {
    ...
    textResponse.style.display = hideCodeResponse ? "block" : "none";
    response.style.display = hideCodeResponse ? "none" : "block";
  },
  enableButtons: (enable) => {
    const askButton = document.getElementById("ask");
    if (askButton) {
      askButton.disabled = !enable;
    }
    const clearButton = document.getElementById("clear");
    if (clearButton) {
  ...
  setResponse: (completion) => {
    const isStringFalsy = !completion;
    if (isStringFalsy) {
      SidebarView.toggleResponse({ hideAll: true });
      return;
    }
    const isRenderedAsText = SidebarView.shouldRenderAsText(completion);
    const textResponse = document.getElementById("text-response");
    const response = document.getElementById("code-body");
    if (textResponse === null || response === null) {
      return;
    }
    if (isRenderedAsText) {
      textResponse.value = completion;
    } else {
      const jsonObject = JSON.parse(completion);
      if (jsonObject[0]) {
        const codeElement = SidebarView.createDOMfromJSON(jsonObject[0]);
        response.replaceChildren(codeElement);
      }
    }
    SidebarView.toggleResponse({ hideCodeResponse: isRenderedAsText });
  },
  createDOMfromJSON: (jsonObject) => {
    let element;
    if (jsonObject.type === "element") {
      element = document.createElement(jsonObject.tagName || "div");
      for (const attr2 of jsonObject.attributes || []) {
        if (attr2.key !== "style") {
          element.setAttribute(attr2.key, attr2.value);
        } else {
          const styles = attr2.value.split(";");
      ...
      if (jsonObject.children) {
        for (let i = 0; i < jsonObject.children.length; i++) {
          const child = SidebarView.createDOMfromJSON(jsonObject.children[i]);
          element.appendChild(child);
        }
      }
    } else if (jsonObject.type === "text") {
      const textAreaElement = document.createElement("textarea");
      textAreaElement.innerHTML = jsonObject.content || "";
      element = document.createTextNode(
        textAreaElement.childNodes.length === 0 ? "" : textAreaElement.childNodes[0].nodeValue || ""
      );
    }
    if (element) {
      return element;
    } else {
      throw new Error("Response can not be displayed.");
    }
  },
  handleIconSelection: (event) => {
    const isSidebarClickEvent = "isTrusted" in event;
    if (isSidebarClickEvent) {
      SidebarView.selectFromSidebarClick(event);
      return;
    }
    SidebarView.selectFromHistoryEvent(event);
  },
  selectFromSidebarClick: (event) => {
    const btnClicked = event.target;
    const parentNode = btnClicked.parentNode;
    if (parentNode) {
      Array.from(parentNode.children).forEach((node) => {
        if (node === event.target) {
          node.classList.toggle("active-icon");
        } else {
          node.classList.remove("active-icon");
        }
      });
    }
  },
  selectFromHistoryEvent: (message) => {
    const prompt = message.data;
    const btnClicked = SidebarView.getEventTarget(prompt.feedback?.reaction);
    const parentNode = btnClicked.parentNode;
    if (parentNode) {
      Array.from(parentNode.children).forEach((node) => {
        if (node === btnClicked) {
  ...
  getEventTarget: (command) => {
    const btnToToggle = document.querySelector(`[data-reaction="${command}"]`);
    return btnToToggle;
  },
  isJSON: (str) => {
    try {
  ...
  shouldRenderAsText: (completion) => {
    return !SidebarView.isJSON(completion);
  }
};
(function() {
  SidebarView.init();
